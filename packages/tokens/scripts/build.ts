...
/**
 * Tailwind v4 preset. `@theme inline` maps utility namespaces onto Notsho variables,
 * so `bg-accent`, `text-muted`, `rounded-control`, `p-inset`, `shadow-raised` resolve
 * to the same runtime-themable custom properties as the components.
 */
export function compileTailwind(m: Manifest): string {
  const sem = m.tokens.filter((t) => t.tier === "semantic");
  const ns: Record<string, (t: Token) => string | null> = {
    color: (t) => (t.name.startsWith("color.") ? `--color-${t.name.slice(6)}` : null),
    font: (t) => (t.name.startsWith("font.") ? `--font-${t.name.slice(5)}` : null),
    radius: (t) => (t.name.startsWith("radius.") ? `--radius-${t.name.slice(7)}` : null),
    shadow: (t) => (t.name.startsWith("shadow.") ? `--shadow-${t.name.slice(7)}` : null),
    spacing: (t) => (t.name.startsWith("space.") ? `--spacing-${t.name.slice(6)}` : null),
    size: (t) => (t.name.startsWith("size.") ? `--spacing-${t.name.slice(5)}` : null),
    text: (t) => (t.name === "text.body-size" ? `--text-body` : null),
    ease: (t) => (t.name.startsWith("motion.easing-") ? `--ease-${t.name.slice(14)}` : null),
  };
  const lines: string[] = [];
  for (const t of sem) for (const f of Object.values(ns)) { const k = f(t); if (k) { lines.push(`  ${k}: var(${t.cssVar});`); break; } }
  // Primitive type scale and weights are static, so they can map directly.
  for (const t of m.tokens.filter((x) => x.tier === "primitive" && x.name.startsWith("size."))) lines.push(`  --text-${t.name.slice(5)}: var(${t.cssVar});`);
  for (const t of m.tokens.filter((x) => x.tier === "primitive" && x.name.startsWith("weight."))) lines.push(`  --font-weight-${t.name.slice(7)}: var(${t.cssVar});`);
  return [
    `/* Generated by @${m.prefix}/tokens v${m.version}. Tailwind v4 preset — import after tokens.css: */`,
    `/*   @import "tailwindcss"; @import "@${m.prefix}/tokens/tailwind.css"; */`,
    `@theme inline {`,
    `  --color-*: initial;`,
    ...lines,
    `}`,
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  writeFileSync(join(DIST, "tailwind.css"), compileTailwind(manifest));
